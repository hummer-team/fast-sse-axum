use crate::auth_middle::{AuthConfig, auth};
use crate::common::sse_common::get_env_var;
use crate::message_compression;
use crate::message_package::EventPackage;
use crate::response_builder::ResponseBuilder;
use crate::sse_event_publish;
use crate::sse_event_publish::EventPublisher;
use crate::{common::sse_common::ResourceResponse, common::sse_common::now_time_with_format};
...
pub async fn mock_test_producer_event(Json(params): Json<MockParams>) -> Response {
    ...
    match publish.publish_event(event_package).await {
        ...
        Err(e) => {
            ...
            let response_data = ResourceResponse::<serde_json::Value>::error("PUBLISH_ERROR", &e);
...
fn get_payload(message: &EventPackage) -> String {
    ...
    let final_payload = if message_compression::is_compression_enabled() && !is_compressed_payload {
    ...
    Fut: std::future::Future<Output = ()> + Send,
{
    ...
    let push_to_dlq = get_env_var("DEAD_MESSAGE_PUSH_DEAD_QUEUE", Some("false")).unwrap_or(false);
